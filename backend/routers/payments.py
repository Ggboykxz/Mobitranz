...
async def initiate_payment(data: PaymentCreate, db: AsyncSession = Depends(get_db)):
    ...
    if data.method == "moovmoney":
        api_response = await payment_service.initiate_moovmoney_payment(
            phone=data.phone_number, amount=trip.amount, reference=payment.id
        )
    ...
    logger.info(
        "Paiement initiated", payment_id=payment.id, trip_id=trip.id, method=data.method
    )
...
async def payment_webhook(data: PaymentWebhook, db: AsyncSession = Depends(get_db)):
    ...
    if not payment:
        logger.warning(
            "Webhook: paiement non trouvé", transaction_id=data.transaction_id
        )
    ...
    if data.status == "success":
        await payment_service.mark_payment_completed(
            db=db, payment=payment, transaction_id=data.transaction_id
        )

        # Mettre à jour le trajet
        trip_result = await db.execute(select(Trip).where(Trip.id == payment.trip_id))
        ...
        await payment_service.mark_payment_failed(
            db=db, payment=payment, reason=data.message or "Échec via webhook"
        )
