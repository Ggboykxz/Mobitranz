...
async def create_trip(data: TripCreate, db: AsyncSession = Depends(get_db)):
    ...
    """
    result = await db.execute(select(Driver).where(Driver.id == data.driver_id))
    driver = result.scalar_one_or_none()

    if not driver:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Conducteur non trouvé"
        )
...
async def get_trip(trip_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if not trip:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Trajet non trouvé"
        )
...
async def start_trip(trip_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if trip.status != TripStatus.PAYMENT_PENDING:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Paiement non confirmé"
        )
...
async def get_active_trip(driver_id: str, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(Trip).where(
            Trip.driver_id == driver_id, Trip.status == TripStatus.ACTIVE
        )
    )
    trip = result.scalar_one_or_none()
...
async def join_trip(trip_id: str, client_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if trip.status not in [TripStatus.PROPOSING, TripStatus.HORN_PENDING]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Trajet non joignable"
        )

    if client_id in trip.client_ids:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Déjà dans ce trajet"
        )

    if len(trip.client_ids) >= trip.seats_count:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Trajet complet"
        )
...
async def leave_trip(trip_id: str, client_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if trip.status == TripStatus.ACTIVE:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Trajet déjà commencé"
        )

    if client_id not in trip.client_ids:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Pas dans ce trajet"
        )
...
async def cancel_trip(
    ...
    if trip.status == TripStatus.COMPLETED:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Trajet déjà terminé"
        )
