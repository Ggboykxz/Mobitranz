...
class Settings(BaseSettings):
    ...
    moovmoney_api_url: Optional[str] = Field(
        default=None,
        alias="MOOVMONEY_API_URL"
    )
    moovmoney_api_key: Optional[str] = Field(
        default=None,
        alias="MOOVMONEY_API_KEY"
    )
    moovmoney_api_secret: Optional[str] = Field(
        default=None,
        alias="MOOVMONEY_API_SECRET"
    )
    
    # Configuration Airtel Money (API Gabon)
    airtelmoney_api_url: Optional[str] = Field(
        default=None,
        alias="AIRTELMONEY_API_URL"
    )
    airtelmoney_api_key: Optional[str] = Field(
        default=None,
        alias="AIRTELMONEY_API_KEY"
    )
    airtelmoney_api_secret: Optional[str] = Field(
        default=None,
        alias="AIRTELMONEY_API_SECRET"
    )
    ...
    rate_limit_auth_requests: int = Field(
        default=5,
        alias="RATE_LIMIT_AUTH_REQUESTS"
    )
    rate_limit_auth_window: int = Field(
        default=900,
        alias="RATE_LIMIT_AUTH_WINDOW"
    )
    rate_limit_payment_requests: int = Field(
        default=10,
        alias="RATE_LIMIT_PAYMENT_REQUESTS"
    )
    rate_limit_payment_window: int = Field(
        default=60,
        alias="RATE_LIMIT_PAYMENT_WINDOW"
    )
    rate_limit_general_requests: int = Field(
        default=100,
        alias="RATE_LIMIT_GENERAL_REQUESTS"
    )
