...
class HornDetectionService:
    ...
    def detect_frequency(
        ...
        peaks, _ = signal.find_peaks(np.abs(spec), height=np.max(np.abs(spec)) * 0.1)
    ...
    def analyze_chunk(self, audio_chunk: np.ndarray, sample_rate: int = None) -> dict:
        ...
        if energy < self.ENERGY_THRESHOLD:
            return {"is_horn": False, "frequency": 0.0, "confidence": 0.0}
        ...
        return {"is_horn": False, "frequency": frequency, "confidence": 0.0}

    def estimate_duration_ms(
        self, audio_samples: np.ndarray, sample_rate: int = None
    ) -> float:
    ...
    def is_acceptance_pattern(self, detected_events: list) -> bool:
    ...
    def is_refus_pattern(self, detected_events: list) -> bool:
