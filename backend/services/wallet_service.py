...
class WalletOperation(Base):
    ...
    operation_type = Column(Enum(WalletOperationType), nullable=False)
...
class WalletService:
    ...
    async def create_wallet(self, db: AsyncSession, user_id: str) -> Wallet:
        ...
        """
        wallet = Wallet(
            user_id=user_id, balance=0, frozen_balance=0, status=WalletStatus.ACTIVE
        )
    ...
    async def get_wallet(self, db: AsyncSession, user_id: str) -> Optional[Wallet]:
    ...
    async def deposit(
        ...
        logger.info(
            "Dépôt effectué", user_id=user_id, amount=amount, reference=reference
        )
    ...
    async def withdraw(
        ...
        if not wallet or wallet.balance < amount:
            logger.warning(
                "Fonds insuffisants pour retrait", user_id=user_id, amount=amount
            )
            return None
        ...
        logger.info(
            "Retrait effectué", user_id=user_id, amount=amount, reference=reference
        )
    ...
    async def freeze_funds(
        ...
        logger.info(
            "Fonds bloqués", user_id=user_id, amount=amount, reference=reference
        )
    ...
    async def release_funds(
        ...
        logger.info(
            "Fonds transférés", from_user=user_id, to_user=to_user_id, amount=amount
        )
