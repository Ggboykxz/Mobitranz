...
class GeoService:
    ...
    def calculate_eta(self, distance_km: float, average_speed_kmh: float = 30.0) -> int:
    ...
    def _is_point_in_polygon(self, lat: float, lon: float, polygon: str) -> bool:
            ...
            for i in range(1, n + 1):
                ...
                if lon > min(p1_lon, p2_lon):
                    if lon <= max(p1_lon, p2_lon):
                        if p1_lat != p2_lat:
                            lat_inters = (lon - p1_lon) * (p2_lat - p1_lat) / (
                                p2_lon - p1_lon
                            ) + p1_lat
                            if lat > lat_inters:
                                inside = not inside
        ...
        for driver in drivers:
            if driver.current_lat and driver.current_lon:
                distance = self.calculate_distance(
                    latitude, longitude, driver.current_lat, driver.current_lon
                )
    ...
    def detect_route_deviation(
        ...
        for point in route_points:
            distance_m = (
                self.calculate_distance(current_lat, current_lon, point[0], point[1])
                * 1000
            )
